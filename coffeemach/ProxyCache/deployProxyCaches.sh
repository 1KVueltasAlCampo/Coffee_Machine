...
rutaArchivo="src/main/resources/proxycache.cfg"

# Ruta al directorio donde se encuentra el proyecto
rutaProyecto="C:\Users\Carvi\Documents\VS WorkSpace\Coffee_Machine\coffeemach\ProxyCache"
...
do
    ...
    powershell.exe -Command "Start-Process cmd.exe -ArgumentList '/c \"cd /d \"$rutaProyecto\" && gradle build && java -jar build/libs/ProxyCache.jar\"'"
...
nuevaLinea=$(sed "s/-p $numero/-p 9095/" $rutaArchivo)
echo "$nuevaLinea" > $rutaArchivo
